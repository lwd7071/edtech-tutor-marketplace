...
- Migration mới nhất trong repository: `V36__enable_rls_communication_tables.sql` (RLS default-deny và REVOKE TRUNCATE trên 4 bảng Communication & Notifications: `conversations`, `messages`, `attachments`, `notifications`). Đã preflight sạch và apply Supabase thành công; Flyway validate/info xác nhận V36 `Success`.
