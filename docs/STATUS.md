...
- RLS V32 preparation (2026-09-14): grant/role preflight SQL và append-only migration đã thêm; clean Flyway migration V1→V32 và RLS behavior tests `16/16` pass (`FlywayMigrationTest` 14/14, `RlsBehaviorVerificationTest` 2/2) trên Docker PostgreSQL. Script production yêu cầu xác nhận backup và quyền apply của Đông. Supabase production preflight, backup, apply và Security Advisor chưa chạy; không gọi là deployed.
