...
- **Database:** V29 fail-fast với dữ liệu legacy mâu thuẫn, thêm `transferred_at`, partial unique indexes cho request đang active và bảng `finance_command_receipts` làm nền cho idempotency.
- **Hệ quả:** Không tự map/xóa status cũ; phải đối soát proof/ledger/audit trước khi retry migration. Command executor và các test concurrency/rollback đầy đủ vẫn là phần triển khai tiếp theo.
