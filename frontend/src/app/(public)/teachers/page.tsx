...
import type {TeacherSearchParams} from '@/shared/api/public';
...
export default async function Page({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 const params:TeacherSearchParams={keyword:value('keyword'),subjectId:value('subjectId'),minPrice:number('minPrice'),maxPrice:number('maxPrice'),minRating:number('minRating'),deliveryMode:value('deliveryMode'),provinceCode:value('provinceCode'),wardCode:value('wardCode'),sort:value('sort'),page:Math.max(0,Math.floor(number('page')||1)-1),size:12};
 ...
 const [teachers,subjects]=await Promise.allSettled([getPublicTeachersServer(params),getPublicSubjectsServer({size:100})]);
