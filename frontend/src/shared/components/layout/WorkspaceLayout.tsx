...
import { workspaceLinks } from '@/shared/lib/navigation';
export default function WorkspaceLayout({ role, children }: { role: UserRole; children: React.ReactNode }) {
 ...
 const active = [...links].sort((a,b) => b.href.length-a.href.length).find(item => pathname === item.href || (item.href.split('/').length>2 && pathname.startsWith(item.href+'/')));
