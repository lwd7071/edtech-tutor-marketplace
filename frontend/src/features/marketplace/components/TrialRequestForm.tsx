...
export default function TrialRequestForm({teacherId,subjects}:{teacherId:string;subjects:{id:string;name:string}[]}){
 const {user}=useAuthStore();const router=useRouter();const path=usePathname();const {message}=App.useApp();const queryClient=useQueryClient();const [busy,setBusy]=useState(false);const [sent,setSent]=useState(false);
