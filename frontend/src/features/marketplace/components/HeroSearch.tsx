...
export default function HeroSearch({subjects=[]}:{subjects?:SubjectSummary[]}) {
 ...
 return <form className="tm-search-form" suppressHydrationWarning onSubmit={e=>{e.preventDefault();const query=new URLSearchParams();if(subject)query.set('subjectId',subject);if(mode)query.set('deliveryMode',mode);if(price)query.set('maxPrice',price);const search=query.toString();router.push(search?'/teachers?'+search:'/teachers');}}>
