...
export default function AssignmentList({role}:{role:'student'|'teacher'}){
 const [page,setPage]=useState(0);const q=useQuery({queryKey:['assignments',role,page],queryFn:()=>role==='teacher'?learningApi.getTeacherAssignments(page,12):learningApi.getStudentAssignments(page,12,'PUBLISHED')});
