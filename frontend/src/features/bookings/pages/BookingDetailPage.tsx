...
import { formatVietnamDateTime, formatVietnamTime } from '@/shared/lib/vietnamTime';
...
export default function BookingDetailPage({ role }: { role: 'student' | 'teacher' }) {
  ...
  const review = useQuery({
    ...
    enabled: role === 'student' && booking.data?.data?.canReview === true,
  });
  ...
  const canComplete = role === 'teacher' && (
    (item.status === 'SCHEDULED' && now >= new Date(item.endTime).getTime()) ||
    (!item.trial && item.settlementStatus === 'REOPENED' && item.canConfirm && !item.sessionReport)
  );
  ...
  return <div className="tm-stack">
    ...
    <section className="tm-panel">
      ...
      <p>{formatVietnamDateTime(item.startTime)} – {formatVietnamTime(item.endTime)}</p>
      <p>Trạng thái: {labels[item.status]}</p>
      {item.outsideAvailabilityWarning && <Alert type="warning" showIcon title="Buổi học nằm ngoài lịch rảnh đã công bố" />}
      {!item.trial && item.settlementStatus && <div style={{ margin: '12px 0' }}><Typography.Text>Quyết toán: <Tag color={item.settlementStatus === 'RELEASED' ? 'green' : item.settlementStatus === 'HELD' ? 'orange' : 'blue'}>{settlementLabels[item.settlementStatus]}</Tag></Typography.Text><br /><Typography.Text type="secondary">Hạn xác nhận: {settlement?.confirmationDeadline ? formatVietnamDateTime(settlement.confirmationDeadline) : '—'} · Gia sư: {settlement?.teacherConfirmedAt ? 'đã xác nhận' : 'chưa xác nhận'} · Học viên: {settlement?.studentConfirmedAt ? 'đã xác nhận' : 'chưa xác nhận'}</Typography.Text></div>}
      ...
      {canComplete && <Button type="primary" onClick={() => setAction('complete')}>{item.settlementStatus === 'REOPENED' ? 'Xác nhận lại và nộp báo cáo' : 'Hoàn thành và viết báo cáo'}</Button>}
      {role === 'teacher' && item.status === 'SCHEDULED' && <Button onClick={() => setAction('cancel')}>Hủy buổi học</Button>}
      {role === 'teacher' && item.canDispute && <Button onClick={() => setDisputeOpen(true)}>Khiếu nại khoản tiền đang giữ</Button>}
      {role === 'student' && item.canReview && !review.isLoading && !currentReview && <Button onClick={() => setAction('review')}>Đánh giá buổi học</Button>}
