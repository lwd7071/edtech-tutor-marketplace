...
class StudentRefundControllerTest {
    ...
    @InjectMocks private StudentRefundController controller;
    ...
    @Test
    void createRefund_shouldReturn201() throws Exception {
        UUID pkgId = UUID.randomUUID();
        CreateRefundRequest req = new CreateRefundRequest(
                pkgId, "Reason", 5, "VCB", "970436", "0123456789", "NGUYEN VAN A"
        );

        RefundRequestView view = new RefundRequestView(
                UUID.randomUUID(), pkgId, studentId, "Reason", 5, 0, null,
                RefundStatus.PENDING, null, "VCB", "970436", "******6789", "NGUYEN VAN A",
                null, null, null, null, 0L, Instant.now()
        );
        when(refundService.createRefund(eq(studentId), any())).thenReturn(view);

        mockMvc.perform(post("/api/student/refund-requests")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(req)))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.requestedSessions").value(5));
    }

    @Test
    void listRefunds_shouldReturn200Paged() throws Exception {
        RefundRequestView view = new RefundRequestView(
                UUID.randomUUID(), UUID.randomUUID(), studentId, "Reason", 5, 0, null,
                RefundStatus.PENDING, null, "VCB", "970436", "******6789", "NGUYEN VAN A",
                null, null, null, null, 0L, Instant.now()
        );
        when(refundService.findStudentRefunds(eq(studentId), any()))
                .thenReturn(new PageImpl<>(List.of(view)));

        mockMvc.perform(get("/api/student/refund-requests")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data[0].requestedSessions").value(5))
