...
class JpaPaymentTransactionRepository implements PaymentTransactionRepository {
    ...
    public Optional<PaymentTransaction> findByProviderReference(String ref) {
        return entityManager.createQuery("select t from PaymentTransaction t where t.providerReference=:ref", PaymentTransaction.class)
                .setParameter("ref", ref).getResultList().stream().findFirst();
    }
}
