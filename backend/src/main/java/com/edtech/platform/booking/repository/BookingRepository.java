...
public interface BookingRepository extends JpaRepository<Booking,UUID> {
 ...
 @Query("select b from Booking b where b.status=com.edtech.platform.booking.domain.BookingStatus.SCHEDULED and b.trial=false and b.endTime <= :cutoff") Page<Booking> findPaidExpiryCandidates(java.time.Instant cutoff, Pageable pageable);
 @Query("select b from Booking b where b.status=com.edtech.platform.booking.domain.BookingStatus.SCHEDULED and b.trial=true and b.endTime <= :cutoff") Page<Booking> findTrialExpiryCandidates(java.time.Instant cutoff, Pageable pageable);
 @Query("select b from Booking b where b.status=com.edtech.platform.booking.domain.BookingStatus.SCHEDULED and b.endTime > :from and b.endTime <= :to") Page<Booking> findReminderCandidates(java.time.Instant from, java.time.Instant to, Pageable pageable);
    ...
    @Query("""
            select count(b) from Booking b left join BookingSettlement s on s.bookingId = b.id
            where b.teacherId = :teacherId
              and b.status = com.edtech.platform.booking.domain.BookingStatus.COMPLETED
              and b.deleted = false
              and (b.trial = true or s.id is null or s.status = com.edtech.platform.booking.domain.SettlementStatus.RELEASED)
            """)
